...
import { collectTasks, AggregatedData, TaskEntry } from "./collector.js";
...
interface AlertConfig {
  type: "cost_threshold" | "failure_rate" | "usage_spike";
  threshold: number;
  enabled: boolean;
}
 ...
 */
async function getData(forceRefresh = false): Promise<AggregatedData> {
  const now = Date.now();
 ...
 */
function toCSV(data: AggregatedData): string {
  const headers = [
    "task_id",
    "content",
    "status",
 ...
 */
function calculateMetrics(data: AggregatedData): MetricsResponse {
  ...
  return {
    ...
    completionRate:
      totalTasks > 0 ? Math.round((completed / totalTasks) * 1000) / 10 : 0,
    tokenUsage: {
 ...
 */
function checkAlerts(data: AggregatedData): AlertResult[] {
...
async function handleRequest(
  ...
  try {
    ...
    if (path === "/api/tasks" || path === "/tasks") {
      ...
      let tasks: TaskEntry[] = data.recentTasks;

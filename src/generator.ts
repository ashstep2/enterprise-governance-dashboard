...
function formatCompact(n: number): string {
  ...
  if (n >= 1_000) return formatWithCommas(n);
  return n.toString();
}
...
function healthIndicator(data: AggregatedData): { label: string; color: string; dotColor: string } {
  ...
  if (failRate > 0.2) return { label: "Critical", color: "text-red-600", dotColor: "bg-red-500" };
  if (failRate > 0.1 || completionRate < 0.5) return { label: "Warning", color: "text-amber-600", dotColor: "bg-amber-400" };
  return { label: "Healthy", color: "text-emerald-600", dotColor: "bg-emerald-500" };
}

function generateStatusBars(statusCounts: Record<string, number>, total: number): string {
  if (total === 0) {
...
function generatePatternsList(patterns: { key: string; count: number }[]): string {
...
function generateRecentTable(entries: AggregatedData["recentTasks"]): string {
  if (entries.length === 0) {
...
function generateModelBreakdown(modelCounts: Record<string, number>): string {
...
export function generateReport(data: AggregatedData): string {

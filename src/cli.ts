...
import { collectTasks, AggregatedData } from "./collector.js";
...
function printHeader(): void {
  ...
  console.log(
    color(
      "║     Analytics • Compliance • Cost Management                 ║",
      "cyan"
    )
  );
  console.log(
...
function printHelp(): void {
  ...
  console.log();
  console.log(
    `  ${color("report", "green")}     Generate HTML governance report`
  );
...
async function showMetrics(data: AggregatedData): Promise<void> {
...
async function showCosts(data: AggregatedData): Promise<void> {
...
async function showModels(data: AggregatedData): Promise<void> {
  console.log();
  console.log(color("═══ MODEL USAGE ═══", "bold"));
  console.log();

  const models = Object.entries(data.modelCounts).sort(
    ([, a], [, b]) => b - a
  );
  ...
  if (models.length === 0) {
    ...
    for (const [model, count] of models) {
      ...
      console.log(
        `  ${model.padEnd(35)} ${String(count).padStart(5)} (${formatPercent(pct).padStart(6)}) ${color(bar, "cyan")}`
      );
    }
  }
  console.log();
...
async function showStatus(data: AggregatedData): Promise<void> {
  console.log();
  console.log(color("═══ STATUS DISTRIBUTION ═══", "bold"));
  console.log();

  const statuses = Object.entries(data.statusCounts).sort(
    ([, a], [, b]) => b - a
  );
  ...
  for (const [status, count] of statuses) {
    ...
    console.log(
      `  ${status.padEnd(15)} ${String(count).padStart(5)} (${formatPercent(pct).padStart(6)}) ${color(bar, c)}`
    );
  }
  console.log();
}

async function showPatterns(data: AggregatedData): Promise<void> {
  console.log();
...
async function checkAlerts(data: AggregatedData): Promise<void> {
  console.log();
  ...
  if (alerts.length === 0) {
    ...
    for (const alert of alerts) {
      ...
      console.log(
        `  ${color(icon, c)} [${color(alert.severity, c)}] ${alert.type}: ${alert.message}`
      );
    }
  }
  console.log();
...
async function generateReportFile(data: AggregatedData): Promise<void> {
  await mkdir(OUTPUT_DIR, { recursive: true });
...
async function interactiveMode(): Promise<void> {
  ...
  console.log(
    `Loaded ${color(String(data.totalEntries), "cyan")} tasks from ${color(String(data.totalFiles), "cyan")} sessions`
  );
  if (isDemo) {

...
export async function collectTasks(customDir) {
    ...
    try {
        ...
        console.warn(`Warning: Could not read ${TASKS_DIR}. Directory may not exist.`);
        console.warn("Generating report with empty data. Create tasks first.");
    }
    ...
    const recentTasks = withTimestamp.length > 0
        ? withTimestamp
        ...
        : allEntries.slice(-20);
    // Estimate cost
    const estimatedCost = (totalTokensInput / 1_000_000) * COST_PER_M_INPUT +
